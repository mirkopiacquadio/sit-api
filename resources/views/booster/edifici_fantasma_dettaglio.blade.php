...
<head>
    ...
    <title>Dettaglio Edifici Fantasma - Booster</title>
    ...
    <div class="container-fluid py-4">
        ...
        <div class="row mb-4">
            <div class="col-12">
                <div class="d-flex justify-content-between align-items-center">
                    <div>
                        <h2>👻 Dettaglio Edifici Fantasma</h2>
                        @php
                            $raw = str_replace('edifici_fantasma_finali_', '', $table);
                            $parts = explode('_', $raw);
                    ...
                    <div class="btn-group">
                        ...
                        <a href="{{ route('booster.index') }}" class="btn btn-secondary">
                            ← Torna al Booster
                        </a>
                    </div>
                </div>
        ...
        @php
            ...
            $sortTh = function ($label, $field) use ($sort, $dir, $code_comune, $table) {
                ...
                $url     = route('booster.ef.dettaglio', [
                ...
                <table class="table table-hover table-striped mb-0">
                    <thead class="table-dark">
                        <tr>
                            ...
                            <th style="white-space:nowrap;">{!! $sortTh('TIPO FANTASMA', 'tipo_fantasma') !!}</th>
                            <th style="white-space:nowrap;">{!! $sortTh('DESCR', 'descr') !!}</th>
                            <th class="text-end" style="white-space:nowrap;">{!! $sortTh('SUPERFICIE (m²)', 'area_mq') !!}</th>
                    ...
                    <tbody>
                        @forelse($rows as $row)
                            ...
                            <tr class="{{ $row->lavorato ? 'riga-lavorata' : '' }}" data-row-id="{{ $row->gid }}">
                                ...
                                <td>
                                    @if($row->tipo_fantasma == 'AMPLIAMENTO')
                                        <span class="badge bg-warning text-dark badge-custom">{{ $row->tipo_fantasma }}</span>
                                    @elseif($row->tipo_fantasma == 'NUOVA EDIFICAZIONE')
                                        <span class="badge bg-primary badge-custom">{{ $row->tipo_fantasma }}</span>
                                    @else
                                        <span class="badge bg-secondary badge-custom">{{ $row->tipo_fantasma ?? '—' }}</span>
                                    @endif
                                </td>
                                <td>{{ $row->descr }}</td>
                                <td class="text-end">{{ number_format($row->area_mq ?? 0, 2, ',', '.') }}</td>
